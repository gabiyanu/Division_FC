...
Usage:
    ...
    comp_id, season_id = loader.find_competition("FIFA World Cup", "2022")
...
# =============================================================================
# PLAYER NAME MAPPING - StatsBomb uses full legal names
# =============================================================================
...
def get_display_name(full_name: str) -> str:
    """
    Get recognizable display name from StatsBomb full legal name.
    
    Args:
        full_name: Full player name from StatsBomb (e.g., "Lionel Andrés Messi Cuccittini")
    
    Returns:
        Display name (e.g., "Messi")
    """
    if not full_name:
        return "Unknown"
    
    # Check if last name is in known names
    parts = full_name.split()
    last_name = parts[-1] if parts else full_name
    
    if last_name in KNOWN_NAMES:
        return KNOWN_NAMES[last_name]
    
    # For unknown players, return last name
    return last_name


def get_short_name(full_name: str, max_chars: int = 12) -> str:
    """
    Get shortened name for visualizations.
...
class StatsBombLoader:
    ...
    def find_competition(
        ...
        Args:
            ...
            exclude_youth: Exclude U20/U17/Women's competitions (default True)
        ...
        # Exclude youth/women's competitions by default
        if exclude_youth:
            comps = comps[~comps['competition_name'].str.contains('U20|U17|U21|U19|Women|Youth', case=False, na=False)]
...
def get_goals(events: pd.DataFrame, regulation_only: bool = False) -> pd.DataFrame:
    """
    Extract all goals from events - sourced from data.
    
    Args:
        events: Match events DataFrame
        regulation_only: If True, only return goals in regulation time (<=90 min)
    
    Returns:
        DataFrame with goal events
    """
    goals = events[
        (events['type_name'] == 'Shot') &
        (events['outcome_name'] == 'Goal')
    ][['minute', 'second', 'player_name', 'team_name', 'period', 'x', 'y']].copy()
    
    if regulation_only:
        goals = goals[goals['period'] <= 2]  # Period 1 = 1st half, 2 = 2nd half
